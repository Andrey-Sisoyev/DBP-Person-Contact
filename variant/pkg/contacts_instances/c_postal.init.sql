...
GRANT USAGE ON SEQUENCE postal_addresses_ids_seq TO user_<<$app_name$>>_data_admin;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE contacts__postal              TO user_<<$app_name$>>_data_admin;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE postal_addresses_in_languages TO user_<<$app_name$>>_data_admin;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE postal_addresses              TO user_<<$app_name$>>_data_admin;
GRANT SELECT                         ON TABLE contacts__postal              TO user_<<$app_name$>>_data_reader;
GRANT SELECT                         ON TABLE postal_addresses_in_languages TO user_<<$app_name$>>_data_reader;
GRANT SELECT                         ON TABLE postal_addresses              TO user_<<$app_name$>>_data_reader;
...
CREATE OR REPLACE FUNCTION new_postal_address(par_postal_address_ci postal_address_construction_input) RETURNS integer AS $$
DECLARE
        pa_id integer:= NULL;
...
GRANT EXECUTE ON FUNCTION new_postal_address(par_postal_address_ci postal_address_construction_input)TO user_<<$app_name$>>_data_admin;
...
CREATE OR REPLACE FUNCTION instaniate_contact_as_postal(par_contact_id integer, par_contact_postal_ci contact_postal_construction_input) RETURNS integer AS $$
DECLARE
        cnt integer:= 0;
        p_id integer;
BEGIN
        p_id:= COALESCE( par_contact_postal_ci.mail_receiving_person_id
                       , find_person_by_contact(par_contact_id)
                       );
...
GRANT EXECUTE ON FUNCTION instaniate_contact_as_postal(par_contact_id integer, par_contact_postal_ci contact_postal_construction_input)TO user_<<$app_name$>>_data_admin;
